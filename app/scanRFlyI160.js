...
/**@name 广播RFlyI160 */
function UDPbroadcastRFlyI160(hostname, message, timeout = 500) {
  return new Promise((resolve, reject) => {
...
function parseRFlyI160UDPData(buf) {
...
getIPV4ForHostname().then((addresses) => {
  return Promise.all(
    addresses.map(({ address }) => {
      return UDPbroadcastRFlyI160(
        address,
        encodeRFlyI160UDPData({
          mac: "38dfa06db44c",
          targetIp: "0.0.1.0",
          targetPort: 20108,
          readerPort: 20108,
          readerIp: "192.168.31.119",
          readerMask: "255.255.255.0",
          readerGeteway: "192.168.31.1",
          mode: 1,
          version: "1.1",
        })
      );
